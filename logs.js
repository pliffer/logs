...
const Logs = {
    ...
    */
    init(logPath){
        ...
        else if(!Logs.path) Logs.path = process.cwd() + '/.logs';
    ...
    checkReason(reason){
        ...
        switch(reason.code){
            case 'MODULE_NOT_FOUND':
                ...
                if(~reason.toString().indexOf(process.cwd())){

                    var rootFile = reason.toString().replace('Error: Cannot find module \'', '').replace("'", '');
                    var file     = rootFile.replace(process.cwd(), '');
            ...
            data.forEach(log => {
                ...
                fs.readFile(logPath, 'utf-8', (err, d) => {
                    ...
                    if(!process.env.LUNASTRO_HOST || !process.env.LUNASTRO_LOGS){
                        ...
                        console.log('LUNASTRO_HOST or LUNASTRO_LOGS is not defined');
                    ...
                    request.post({
                        ...
                        if(res && res.success){
                            ...
                            // Ao enviar, o arquivo muda de nome para <nome>.sent.log
                            fs.rename(logPath, logPath.replace('.log', '.sent.log'));
                    ...
                    readArr.push(new Promise((resolve, reject) => {
                        ...
                        fs.readFile(logPath, 'utf-8', (err, data) => {
                            ...
                            if(err) reject(err);
                                ...
                                resolve({
                                    path: logFilePath.replace(process.cwd(), ''),
                                    name: log,
                                    data: data
                                });
                            }
